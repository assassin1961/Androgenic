/* =====================================================================
   Androgenic Jobs — swipe to apply (Tinder-style, minimal)
   Live jobs (Remotive -> Jobicy -> Arbeitnow -> seed; Brazil excluded).
   Swipe right = auto-apply with saved profile; left = skip.
   First apply asks for your profile once; after that it's automatic.
   ===================================================================== */
(() => {
  'use strict';
  ...
  const mapRemotive = j => ({ id:'rm'+j.id, title:j.title, co:j.company_name, loc:cleanLoc(j.candidate_required_location), type:prettyType(j.job_type), remote:'Remote', salary:(j.salary||'').trim(), posted:ago(j.publication_date), tags:(j.tags||[]).slice(0,5), desc:toText(j.description), url:j.url, logo:j.company_logo||'', src:'Remotive' });
  const mapJobicy = j => ({ id:'jc'+(j.id||Math.random().toString(36).slice(2)), title:j.jobTitle, co:j.companyName, loc:cleanLoc(j.jobGeo), type:prettyType((j.jobType&&j.jobType[0])||'full_time'), remote:'Remote', salary:(j.annualSalaryMin&&j.annualSalaryMax)?`$${(+j.annualSalaryMin/1000)|0}k–$${(+j.annualSalaryMax/1000)|0}k`:'', posted:ago(j.pubDate), tags:(j.jobIndustry||[]).slice(0,4), desc:toText(j.jobDescription||j.jobExcerpt), url:j.url, logo:j.companyLogo||'', src:'Jobicy' });
  const mapArbeit = j => ({ id:'ab'+(j.slug||Math.random().toString(36).slice(2)), title:j.title, co:j.company_name, loc:cleanLoc(j.location||(j.remote?'Worldwide':'')), type:prettyType((j.job_types||[])[0]), remote:j.remote?'Remote':'On-site', salary:'', posted:ago(j.created_at?j.created_at*1000:null), tags:(j.tags||[]).slice(0,5), desc:toText(j.description), url:j.url, logo:'', src:'Arbeitnow' });
  ...
  const SEED = [
    { id:'s1', title:'Frontend Engineer', co:'Remote Co', loc:'Worldwide', type:'Full-time', remote:'Remote', salary:'$90k–$130k', posted:'today', tags:['React','TypeScript'], desc:'Build delightful web experiences for a fully-remote team. Own features end to end and ship every week. We care about craft, speed, and a great developer experience.', url:'', src:'Sample' },
    { id:'s2', title:'Product Designer', co:'Studio Nine', loc:'Europe', type:'Full-time', remote:'Remote', salary:'$80k–$110k', posted:'1d ago', tags:['Figma','Prototyping'], desc:'Design clean, usable products from idea to launch. Strong portfolio required. You will run research, prototype quickly, and partner closely with engineering.', url:'', src:'Sample' },
    { id:'s3', title:'Data Analyst', co:'Insight Labs', loc:'USA', type:'Full-time', remote:'Remote', salary:'$70k–$100k', posted:'2d ago', tags:['SQL','Python'], desc:'Turn data into decisions. Build dashboards and answer hard questions with numbers. Work closely with product and leadership to drive impact.', url:'', src:'Sample' },
    { id:'s4', title:'Customer Support Specialist', co:'Helpful Inc', loc:'Worldwide', type:'Full-time', remote:'Remote', salary:'$45k–$60k', posted:'3d ago', tags:['Support'], desc:'Be the friendly voice customers rely on. Empathy and clear writing matter most. Help people get unstuck and turn problems into delight.', url:'', src:'Sample' },
    { id:'s5', title:'DevOps Engineer', co:'Cloudly', loc:'Worldwide', type:'Full-time', remote:'Remote', salary:'$120k–$160k', posted:'4d ago', tags:['AWS','Kubernetes'], desc:'Own the platform that lets engineers ship safely and fast. Automate everything and champion reliability across the stack.', url:'', src:'Sample' },
    { id:'s6', title:'Growth Marketer', co:'Launchpad', loc:'USA', type:'Full-time', remote:'Remote', salary:'$75k–$110k', posted:'5d ago', tags:['SEO','Paid Social'], desc:'Drive growth across channels and obsess over the funnel. Run experiments, measure everything, and scale what works.', url:'', src:'Sample' },
  ];

  /* ---------------- state ---------------- */
  let QUEUE = [], idx = 0, LIVE = false, busy = false, lastAction = null;
  ...
  const saveApplied = () => localStorage.setItem('andro_applied', JSON.stringify(applied.slice(0, 200)));
  const saveSeen = () => localStorage.setItem('andro_seen', JSON.stringify(seen));
  let pendingApply = null; // job awaiting profile setup

  const deck = $('#deck');
  function toast(m, kind=''){ const t = $('#toast'); t.textContent = m; t.className = 'toast show ' + kind; clearTimeout(toast._t); toast._t = setTimeout(() => t.className = 'toast ' + kind, 2000); }
  ...
  function cardHTML(j, pos){
    ...
    return `<article class="deck-card" data-pos="${pos}" data-id="${esc(j.id)}" style="--h:${hue(j.co)}">
      ...
      <div class="card-head">
        ...
        <div class="card-co">${esc(j.co)}</div>
        <div class="card-loc">${ICloc} ${esc(j.loc)} · ${esc(j.posted)}</div>
      </div>
      <div class="card-body">
        <div class="pills">
          ${j.salary?`<span class="pill salary">${esc(j.salary)}</span>`:''}
          <span class="pill">${esc(j.type)}</span><span class="pill">${esc(j.remote)}</span>
          ${j.tags.slice(0,3).map(t=>`<span class="pill accent">${esc(t)}</span>`).join('')}
        </div>
        <div class="card-desc">${esc(j.desc||'Tap apply to send your profile to this role.')}</div>
      </div>
  ...
  function renderDeck(){
    ...
    if (!rest.length) { deck.innerHTML = ''; $('#empty').hidden = false; $('#actions').style.visibility = 'hidden'; $('#hint').style.visibility = 'hidden'; return; }
    $('#empty').hidden = true; $('#actions').style.visibility = ''; $('#hint').style.visibility = '';
    deck.innerHTML = rest.map((j, i) => cardHTML(j, i)).reverse().join(''); // pos2 first, pos0 last (on top)
    bindFront();
  ...
  let drag = null;
  function bindFront(){
    const card = deck.querySelector('.deck-card[data-pos="0"]'); if (!card) return;
    card.addEventListener('pointerdown', onDown);
  }
  function onDown(e){
    if (busy) return;
    const card = e.currentTarget;
    drag = { card, x0:e.clientX, y0:e.clientY, dx:0, dy:0, id:e.pointerId };
  ...
  function onMove(e){
    ...
    const t = Math.min(Math.abs(drag.dx) / 110, 1);
  ...
  function onUp(){
    ...
    const threshold = 100;
    if (dx > threshold) fling('right', card);
    else if (dx < -threshold) fling('left', card);
    else { card.classList.add('animate'); card.style.transform = ''; card.querySelectorAll('.stamp').forEach(s => s.style.opacity = 0); setTimeout(() => card.classList.remove('animate'), 300); }
    drag = null;
  }
  function fling(dir, card){
    card = card || deck.querySelector('.deck-card[data-pos="0"]'); if (!card || busy) return;
    busy = true;
    const j = QUEUE[idx];
    card.classList.add('animate');
    const off = dir === 'right' ? window.innerWidth : -window.innerWidth;
    card.style.transform = `translate(${off}px, ${drag?drag.dy:0}px) rotate(${dir==='right'?22:-22}deg)`;
    card.querySelector(dir==='right'?'.stamp.apply':'.stamp.skip').style.opacity = 1;
    setTimeout(() => { commit(dir, j); busy = false; }, 300);
  }
  function commit(dir, j){
    seen[j.id] = 1; saveSeen();
    lastAction = { dir, idx, id: j.id };
    if (dir === 'right') applyToJob(j);
    else toast('Skipped', 'nope');
    idx++;
    renderDeck();
  }
  ...
  function applyToJob(j){
    ...
    if (!appliedIds().has(j.id)) {
      applied.unshift({ id:j.id, title:j.title, co:j.co, url:j.url||'', loc:j.loc, at:Date.now() });
      saveApplied();
      sendApplication(j);
    }
    $('#appliedCount').textContent = applied.length;
    toast(`Applied to ${j.co} ✓`, 'like');
  }
  function sendApplication(j){
    // background submit into a hidden iframe (no navigation); FormSubmit emails the owner
    const f = document.createElement('form');
    f.method = 'POST'; f.action = 'https://formsubmit.co/' + APPLY_EMAIL; f.target = 'sink'; f.style.display = 'none';
  ...
  function openProfile(forApply){
    ...
    m.innerHTML = `<div class="modal" style="position:relative">
      ...
      <div class="modal-head"><h3>${forApply?'Set up your profile':'Your profile'}</h3><p>${forApply?'Fill this once — then every right-swipe auto-applies with it.':'Used to auto-apply when you swipe right.'}</p></div>
    ...
    setTimeout(() => $('#pf-name') && $('#pf-name').focus(), 60);
  }
  function closeModal(){ const m = $('#modal'); m.classList.remove('open'); m.setAttribute('aria-hidden','true'); m.innerHTML = ''; if (pendingApply) { pendingApply = null; renderDeck(); } }
  function saveProfile(e){
    e.preventDefault();
    const name = $('#pf-name').value.trim(), email = $('#pf-email').value.trim(), phone = $('#pf-phone').value.trim();
    const err = $('#pf-err');
    ...
    const pend = pendingApply; pendingApply = null;
  ...
  function openApplied(){
    ...
    s.innerHTML = `<div class="sheet-head"><h2>Applied · ${applied.length}</h2><button class="ipill" data-closesheet><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button></div>
      <div class="sheet-list">${applied.length ? applied.map(a => `<div class="ai"><span class="lg" style="--h:${hue(a.co)}">${esc(initials(a.co))}</span>
        <div class="info"><b>${esc(a.title)}</b><small>${esc(a.co)} · ${esc(a.loc||'')}</small></div>
        ${a.url?`<a class="open" href="${esc(a.url)}" target="_blank" rel="noopener">Open ↗</a>`:''}</div>`).join('') : '<div class="sheet-empty">No applications yet.<br>Swipe right on a job to apply.</div>'}</div>`;
  ...
  async function load(refresh){
    ...
    if (!QUEUE.length || refresh) loading();
    const live = await fetchLive();
    let jobs = (live && live.length) ? live : SEED;
    LIVE = !!(live && live.length);
    jobs = jobs.filter(j => j.title && j.co && !/brazil|brasil/i.test(`${j.loc} ${j.co} ${j.title}`));
    // de-dupe + put unseen first
    const seenSet = seen;
    jobs.sort((a, b) => (seenSet[a.id]?1:0) - (seenSet[b.id]?1:0));
    QUEUE = jobs; idx = 0; lastAction = null;
    $('#appliedCount').textContent = applied.length;
    renderDeck();
  ...
  $('#btnUndo').addEventListener('click', () => {
    ...
    delete seen[lastAction.id]; saveSeen();
    idx = Math.max(0, lastAction.idx); lastAction = null; renderDeck(); toast('Brought it back');
  });
  $('#refreshBtn').addEventListener('click', () => load(true));
  $('#reloadBtn') && document.addEventListener('click', e => { if (e.target.id === 'reloadBtn') load(true); });
